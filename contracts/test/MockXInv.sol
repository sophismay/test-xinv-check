 ...
 * @author Compound
 */
contract MockInvCore is CTokenInterface, Exponential, TokenErrorReporter {
     ...
     */
    function initializeMock(ComptrollerInterface comptroller_,
    ...
    function mintFresh(address minter, uint mintAmount) internal returns (uint, uint) {
        console.log("inside mint fresh");
        /* Fail if mint not allowed */
        uint allowed = comptroller.mintAllowed(address(this), minter, mintAmount);
        if (allowed != 0) {
            return (failOpaque(Error.COMPTROLLER_REJECTION, FailureInfo.MINT_COMPTROLLER_REJECTION, allowed), 0);
        }
        console.log("mint allowed");
 ...
 * @author Compound
 */
contract MockXInv is MockInvCore, CErc20Interface {
    ...
    constructor(address underlying_,
        ...
        admin = msg.sender;
        console.log("in constructor of MockXInv");
        // CToken initialize does the bulk of the work
        super.initializeMock(comptroller_, 1e18, name_, symbol_, decimals_);
     ...
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function mintXINV(uint mintAmount) external returns (uint) {
    ...
    function transfer(address dst, uint amount) external returns (bool) { return false; }
    function transferFrom(address src, address dst, uint amount) external returns (bool) { return false; }
    function approve(address spender, uint amount) external returns (bool) { return false; }
    function allowance(address owner, address spender) external view returns (uint) { return 0; }
    function borrow(uint borrowAmount) external returns (uint) { return 0; }
    function repayBorrow(uint repayAmount) external returns (uint) { return 0; }
    function repayBorrowBehalf(address borrower, uint repayAmount) external returns (uint) { return 0; }
    function liquidateBorrow(address borrower, uint repayAmount, CTokenInterface cTokenCollateral) external returns (uint)  { return 0; }
    function _addReserves(uint addAmount) external returns (uint) { return 0; }
    function _setReserveFactor(uint newReserveFactorMantissa) external returns (uint) {return 0;}
    function _reduceReserves(uint reduceAmount) external returns (uint) {return 0;}
    function _setInterestRateModel(InterestRateModel newInterestRateModel) public returns (uint) {return 0;}
    function borrowRatePerBlock() external view returns (uint) {return 0;}
    function supplyRatePerBlock() external view returns (uint) {return 0;}
    ...
    function borrowBalanceStored(address account) public view returns (uint) {return 0;}
}